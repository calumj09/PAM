...
struct GrowthTrackingSection: View {
    ...
    @State private var selectedMeasurementType: TrackerType?
    ...
    var headEntries: [TrackerEntry] {
        dataStore.trackerEntries
            .filter { $0.type == .headCircumference }
            .sorted { $0.timestamp < $1.timestamp }
    }
    ...
    var latestHead: Double? {
        headEntries.last?.quantity
    }
    
    var body: some View {
        VStack(spacing: PAMSpacing.lg) {
            // Growth Quick Entry Section
            VStack(alignment: .leading, spacing: PAMSpacing.md) {
                ...
                // 1x3 Grid for growth measurements
                HStack(spacing: PAMSpacing.md) {
                    ForEach(TrackerType.growthMeasurements, id: \.self) { type in
                        GrowthQuickEntryButton(type: type) {
            ...
            VStack(spacing: PAMSpacing.md) {
                ...
                MeasurementCard(
                    ...
                    value: latestHead.map { String(format: "%.1f cm", $0) } ?? "No data",
                    icon: "circle.dashed",
                    color: Color(hex: "#9E9E9E"),
                    trend: calculateTrend(for: headEntries)
                )
            }
            .padding(.horizontal, PAMSpacing.lg)
...
// MARK: - Growth Quick Entry Button
struct GrowthQuickEntryButton: View {
    let type: TrackerType
    let action: () -> Void
    @State private var isPressed = false
    
    var buttonColor: Color {
        return PAMColors.brandRed // Use brand red for all growth tracker buttons
    }
